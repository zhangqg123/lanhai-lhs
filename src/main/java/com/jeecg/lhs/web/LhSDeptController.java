 ...
 */
@Controller
@RequestMapping("/jeecg/lhSDept")
public class LhSDeptController extends BaseController{
  @Autowired
  private LhSDeptService lhSDeptService;
	...
	public void list(@ModelAttribute LhSDeptEntity query,HttpServletRequest request,HttpServletResponse response,
			@RequestParam(required = false, value = "pageNo", defaultValue = "1") int pageNo,
			@RequestParam(required = false, value = "pageSize", defaultValue = "10") int pageSize) throws Exception{
			try {
			 	LOG.info(request, " back list");
			 	//分页数据
				MiniDaoPage<LhSDeptEntity> list =  lhSDeptService.getAll(query,pageNo,pageSize);
	...
	@ResponseBody
	public AjaxJson doAdd(@ModelAttribute LhSDeptEntity lhSDept){
		AjaxJson j = new AjaxJson();
		try {
			lhSDeptService.insert(lhSDept);
			j.setMsg("保存成功");
		} catch (Exception e) {
			j.setSuccess(false);
			j.setMsg("保存失败");
	...
	@ResponseBody
	public AjaxJson doEdit(@ModelAttribute LhSDeptEntity lhSDept){
		AjaxJson j = new AjaxJson();
		try {
			lhSDeptService.update(lhSDept);
			j.setMsg("编辑成功");
		} catch (Exception e) {
			j.setSuccess(false);
			j.setMsg("编辑失败");
	...
	public AjaxJson batchDelete(@RequestParam(required = true, value = "ids") String[] ids) {
		AjaxJson j = new AjaxJson();
		try {
			lhSDeptService.batchDelete(ids);
			j.setMsg("批量删除成功");
		} catch(Exception e) {
			j.setSuccess(false);
			j.setMsg("批量删除失败");
